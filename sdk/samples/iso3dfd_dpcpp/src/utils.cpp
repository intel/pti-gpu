...
bool CheckGridDimension(size_t n1, size_t n2, size_t n3, unsigned int dim_x,
  ...
  if (0 == dim_y) {
    std::cout << " ERROR: Invalid Grid Size: DIMY cannot be Zero\n";
    return true;
  }
  if (0 == block_z) {
...
void PrintStats(double time, size_t n1, size_t n2, size_t n3,
  ...
  if (0 < nIterations) {
    ...
    std::cout << "nIterations  : 0\n";
    std::cout << "time         : Undefined\n";
    std::cout << "throughput   : Undefined\n";
    std::cout << "flops        : Undefined\n";
    std::cout << "bytes        : Undefined\n";
  }
  std::cout << "\n--------------------------------------\n";
  std::cout << "\n--------------------------------------\n";
}
