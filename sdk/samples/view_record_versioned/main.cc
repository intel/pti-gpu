...
/**
 * @brief Demonstrates how to write backward-compatible PTI tracing code that works
 *        across different library versions by checking ptiVersion() at runtime.
 *
 * PTI library evolves its view record structures over time. To maintain compatibility:
 *
 * 1. Query the PTI version at startup:
 *      pti_version version = ptiVersion();
 *
 * 2. Use version checks to select the appropriate record structures:
 *    - Version >= 1.0 or 0.18+: Use v2 structures (pti_view_record_kernel_v2,
 *      pti_view_record_memory_copy_v2, pti_view_record_memory_fill_v2)
 *    - Version < 0.18: Use older structures (pti_view_record_kernel,
 *      pti_view_record_memory_copy, pti_view_record_memory_fill)
 *
 * 3. The version check pattern:
 *      if (version._major >= 1 || (version._major == 0 && version._minor > 17)) {
 *          // Use v2 record structures
 *          auto* rec = reinterpret_cast<pti_view_record_kernel_v2*>(ptr);
 *      } else {
 *          // Use older record structures
 *          auto* rec = reinterpret_cast<pti_view_record_kernel*>(ptr);
 *      }
 *
 * This approach allows application to:
 * - Work with both older and newer PTI library versions
 * - Access new fields in v2 structures when available
 * - Gracefully degrade to older structures on older systems
 *
 * Note: Every new view record is extension of the previous version. Any new fields are added to the
 * end of the structure. This way, when we cast a buffer of records to an older structure, the new
 * fields are simply ignored, ensuring backward compatibility.
 */
...
void ParseBuffer_v2(unsigned char *buf, std::size_t buf_size, std::size_t valid_buf_size) {
  ...
  while (true) {
    ...
    switch (ptr->_view_kind) {
      ...
      case pti_view_kind::PTI_VIEW_DEVICE_GPU_MEM_COPY: {
        ...
        samples_utils::DumpRecord(reinterpret_cast<pti_view_record_memory_copy_v2 *>(ptr));
        std::cout << "---------------------------------------------------"
                     "-----------------------------"
                  << '\n';
        break;
      ...
      case pti_view_kind::PTI_VIEW_DEVICE_GPU_MEM_FILL: {
        std::cout << "---------------------------------------------------"
                  ...
                  << '\n';
        std::cout << "Found Memory Record" << '\n';
        samples_utils::DumpRecord(reinterpret_cast<pti_view_record_memory_fill_v2 *>(ptr));
        std::cout << "---------------------------------------------------"
                     "-----------------------------"
                  << '\n';
        break;
      ...
      case pti_view_kind::PTI_VIEW_DEVICE_GPU_KERNEL: {
        pti_view_record_kernel_v2 *rec = reinterpret_cast<pti_view_record_kernel_v2 *>(ptr);
        std::cout << "---------------------------------------------------"
                     "-----------------------------"
                  << '\n';
...
void ParseBuffer_v1(unsigned char *buf, std::size_t buf_size, std::size_t valid_buf_size) {
  if (!buf || !valid_buf_size || !buf_size) {
    std::cerr << "Received empty buffer" << '\n';
    if (buf) {
      samples_utils::AlignedDealloc(buf);
...
void ParseBuffer(unsigned char *buf, std::size_t buf_size, std::size_t valid_buf_size) {
  // Dispatch to the appropriate parser based on PTI version at runtime:
  // - v2 parser: PTI >= 1.0 or PTI 0.18+ (uses pti_view_record_*_v2 structures)
  // - v1 parser: PTI < 0.18 (uses older pti_view_record_* structures)
  if (gPtiVersion._major >= 1 || (gPtiVersion._major == 0 && gPtiVersion._minor > 17)) {
    ParseBuffer_v2(buf, buf_size, valid_buf_size);
  } else {
    ParseBuffer_v1(buf, buf_size, valid_buf_size);
  }
}
...
int main(int argc, char *argv[]) {
  ...
  // Query the PTI library version at runtime to determine which view record
  // structures to use when parsing buffers. Newer versions (>= 0.18) provide
  // v2 structures (e.g., pti_view_record_kernel_v2) with additional fields over
  // older version structures. Store the version globally so the buffer parsing
  // callback can select the appropriate record types.
  gPtiVersion = ptiVersion();
