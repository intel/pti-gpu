...
def gen_api(f, func_list, kfunc_list, exclude_from_epilogue_list, exclude_from_prologue_list):
  ...
  for func in func_list:
    f.write("    zelTracer" + func[2:] + "RegisterCallback(" + "tracer, ZEL_REGISTER_PROLOGUE, " + func + "OnEnter);\n")
    f.write("    zelTracer" + func[2:] + "RegisterCallback(" + "tracer, ZEL_REGISTER_EPILOGUE, " + func + "OnExit);\n")
  f.write("  }\n")
...
def gen_exit_callback(f, func, submission_func_list, synchronize_func_list_on_enter, synchronize_func_list_on_exit, hybrid_mode_func_list):
  f.write("  [[maybe_unused]] ZeCollector* collector =\n")
  f.write("    static_cast<ZeCollector*>(global_data);\n")
  if (func not in hybrid_mode_func_list):
    f.write("  if (collector->options_.hybrid_mode) return;\n")
...
def gen_callbacks(f, func_param_dict, submission_func_list, synchronize_func_list_on_enter, synchronize_func_list_on_exit, hybrid_mode_func_list):
  for func in func_param_dict.keys():
    ...
    gen_enter_callback(f, func, synchronize_func_list_on_enter, hybrid_mode_func_list)
    f.write("}\n")
    f.write("\n")
    ...
    f.write("    [[maybe_unused]]void* global_data,\n")
    f.write("    [[maybe_unused]]void** instance_user_data) {\n")
    gen_exit_callback(f, func, submission_func_list, synchronize_func_list_on_enter, synchronize_func_list_on_exit, hybrid_mode_func_list)
    f.write("}\n")
    f.write("\n")
...
def main():
  ...
  l0_file_path_api_1_1_plus = os.path.join(l0_path, "layers","zel_tracing_register_cb.h")
  ...
  gen_callbacks(dst_file, func_param_dictionary, submission_func_list, synchronize_func_list_on_enter, synchronize_func_list_on_exit, hybrid_mode_func_list)
  gen_api(dst_file, func_list, kfunc_list, exclude_from_epilogue_list, exclude_from_prologue_list )
