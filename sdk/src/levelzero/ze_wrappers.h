...
typedef ze_result_t (*fptr_zeEventPoolGetFlags_t) (
    ze_event_pool_handle_t hEventPool,
    ze_event_pool_flags_t* pFlags);

typedef ze_result_t (*fptr_zeCommandListGetDeviceHandle_t) (
    ze_command_list_handle_t command_list,
    ze_device_handle_t* device);

typedef ze_result_t (*fptr_zeCommandListGetContextHandle_t) (
    ze_command_list_handle_t command_list,
    ze_context_handle_t* context);

typedef ze_result_t (*fptr_zeCommandListIsImmediate_t) (
    ze_command_list_handle_t command_list,
    ze_bool_t* isImmediate);

typedef ze_result_t (*fptr_zeCommandListImmediateGetIndex_t) (
    ze_command_list_handle_t command_list,
    uint32_t* index);

typedef ze_result_t (*fptr_zeCommandListGetOrdinal_t) (
    ze_command_list_handle_t command_list,
    uint32_t* ordinal);
typedef ze_result_t (*fptr_zeCommandQueueGetIndex_t) (
    ze_command_queue_handle_t command_queue,
    uint32_t* index);

typedef ze_result_t (*fptr_zeCommandQueueGetOrdinal_t) (
    ze_command_queue_handle_t command_list,
    uint32_t* ordinal);
...
if (l0_loader_handle_) {
  ...
  ze_result_t w_zeEventPoolGetFlags(
      ze_event_pool_handle_t hEventPool,
      ze_event_pool_flags_t* pFlags) {
  ...
  ze_result_t w_zeCommandListGetDeviceHandle(
      ze_command_list_handle_t command_list,
      ze_device_handle_t* device) {
  ...
  ze_result_t w_zeCommandListGetContextHandle(
      ze_command_list_handle_t command_list,
      ze_context_handle_t* context) {
  ...
  ze_result_t w_zeCommandListIsImmediate(
      ze_command_list_handle_t command_list,
      ze_bool_t* isImmediate) {
  ...
  ze_result_t w_zeCommandListImmediateGetIndex(
      ze_command_list_handle_t command_list,
      uint32_t* index) {
  ...
  ze_result_t w_zeCommandListGetOrdinal(
      ze_command_list_handle_t command_list,
      uint32_t* ordinal) {
  ...
  ze_result_t w_zeCommandQueueGetIndex(
      ze_command_queue_handle_t command_queue,
      uint32_t* index) {
  ...
  ze_result_t w_zeCommandQueueGetOrdinal(
      ze_command_queue_handle_t command_queue,
      uint32_t* ordinal) {
...
#define LOAD_AND_DEBUG_PRINT(name) fptr_ ##name## _ = (fptr_ ##name## _t)dlsym( l0_handle_,\
                                                                         #name);\
  ...
  ze_result_t InitIntrospectionWrappers() {
    ...
    if (nullptr == fptr_zeEventPoolGetFlags_ ||
        nullptr == fptr_zeCommandListGetDeviceHandle_ ||
        ...
        nullptr == fptr_zeCommandQueueGetIndex_ ||
        nullptr == fptr_zeCommandQueueGetOrdinal_ ||
  ...
  ze_result_t InitDynamicTracingWrappers() {
    ...
    if (nullptr == fptr_zelEnableTracingLayer_ ||
        nullptr == fptr_zelDisableTracingLayer_ ) {
          return ZE_RESULT_ERROR_UNSUPPORTED_FEATURE;
        }
    return ZE_RESULT_SUCCESS;
  }
