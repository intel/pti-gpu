...
inline std::string GetDeviceVendor(cl_device_id device) {
  ...
  cl_int status = clGetDeviceInfo(device, CL_DEVICE_VENDOR, MAX_STR_SIZE, vendor, nullptr);
...
inline std::vector<cl_device_id> GetDeviceList(cl_device_type type) {
  ...
  for (cl_uint i = 0; i < platform_count; ++i) {
    ...
    status = clGetDeviceIDs(platform_list[i], type, 0, nullptr, &device_count);
    if (status != CL_SUCCESS || device_count == 0) continue;

    std::vector<cl_device_id> device_list(device_count, nullptr);
    status = clGetDeviceIDs(platform_list[i], type, device_count, device_list.data(), nullptr);
    PTI_ASSERT(status == CL_SUCCESS);
...
inline std::vector<cl_device_id> CreateSubDeviceList(cl_device_id device) {
  ...
  status = clCreateSubDevices(device, props, sub_device_count, sub_device_list.data(), nullptr);
  PTI_ASSERT(status == CL_SUCCESS);

  return sub_device_list;
}

inline void ReleaseSubDeviceList(const std::vector<cl_device_id>& sub_device_list) {
...
inline cl_device_id GetDeviceParent(cl_device_id device) {
  ...
  cl_int status =
      clGetDeviceInfo(device, CL_DEVICE_PARENT_DEVICE, sizeof(cl_device_id), &parent, nullptr);
  PTI_ASSERT(status != CL_SUCCESS);
...
inline std::string GetKernelName(cl_kernel kernel, bool demangle = false) {
  ...
  status = clGetKernelInfo(kernel, CL_KERNEL_FUNCTION_NAME, MAX_STR_SIZE, name, nullptr);
  PTI_ASSERT(status == CL_SUCCESS);
...
inline std::string GetDeviceName(cl_device_id device) {
  ...
  char name[MAX_STR_SIZE] = {0};
  cl_int status = clGetDeviceInfo(device, CL_DEVICE_NAME, MAX_STR_SIZE, name, nullptr);
  PTI_ASSERT(status == CL_SUCCESS);
...
inline cl_device_type GetDeviceType(cl_device_id device) {
  ...
  cl_int status = clGetDeviceInfo(device, CL_DEVICE_TYPE, sizeof(type), &type, nullptr);
  PTI_ASSERT(status == CL_SUCCESS);
...
inline cl_program GetProgram(cl_kernel kernel) {
  ...
  status = clGetKernelInfo(kernel, CL_KERNEL_PROGRAM, sizeof(cl_program), &program, nullptr);
  PTI_ASSERT(status == CL_SUCCESS);
...
inline cl_context GetContext(cl_kernel kernel) {
  ...
  status = clGetKernelInfo(kernel, CL_KERNEL_CONTEXT, sizeof(cl_context), &context, nullptr);
  PTI_ASSERT(status == CL_SUCCESS);
...
inline std::vector<cl_device_id> GetDeviceList(cl_program program) {
  ...
  cl_uint device_count = 0;
  status =
      clGetProgramInfo(program, CL_PROGRAM_NUM_DEVICES, sizeof(cl_uint), &device_count, nullptr);
  PTI_ASSERT(status == CL_SUCCESS);
  if (device_count == 0) {
    return std::vector<cl_device_id>();
  }
  ...
  status = clGetProgramInfo(program, CL_PROGRAM_DEVICES, device_count * sizeof(cl_device_id),
...
inline cl_queue_properties* EnableQueueProfiling(const cl_queue_properties* props) {
  ...
  if (props == nullptr) {
    ...
    if (queue_props_id >= 0 && queue_props_id + 1 < props_count) {
      ...
      props_with_prof[queue_props_id + 1] |= static_cast<unsigned long>(CL_QUEUE_PROFILING_ENABLE);
...
inline bool CheckExtension(cl_device_id device, const char* extension) {
  ...
  status = clGetDeviceInfo(device, CL_DEVICE_EXTENSIONS, size, buffer.data(), nullptr);
  PTI_ASSERT(status == CL_SUCCESS);
...
inline size_t GetKernelSimdWidth(cl_device_id device, cl_kernel kernel) {
  ...
  status = clGetDeviceInfo(device, CL_DEVICE_PLATFORM, sizeof(cl_platform_id), &platform, nullptr);
  PTI_ASSERT(status == CL_SUCCESS);
  PTI_ASSERT(platform != nullptr);

  clGetKernelSubGroupInfoKHR func = reinterpret_cast<clGetKernelSubGroupInfoKHR>(
  ...
  size_t simd_width = 0;
  status = func(kernel, device, CL_KERNEL_MAX_SUB_GROUP_SIZE_FOR_NDRANGE_KHR, sizeof(size_t[3]),
                local_size, sizeof(size_t), &simd_width, nullptr);
  PTI_ASSERT(status == CL_SUCCESS);
...
inline cl_command_queue GetCommandQueue(cl_event event) {
  ...
  status = clGetEventInfo(event, CL_EVENT_COMMAND_QUEUE, sizeof(cl_command_queue), &queue, nullptr);
  PTI_ASSERT(status == CL_SUCCESS);
...
inline cl_device_id GetDevice(cl_command_queue queue) {
  ...
  status = clGetCommandQueueInfo(queue, CL_QUEUE_DEVICE, sizeof(cl_device_id), &device, nullptr);
  PTI_ASSERT(status == CL_SUCCESS);
...
inline cl_ulong GetEventTimestamp(cl_event event, cl_profiling_info info) {
  ...
  status = clGetEventProfilingInfo(event, info, sizeof(cl_ulong), &start, nullptr);
  if (status != CL_SUCCESS)
    std::cerr << "\nCannot get timing info for kernel:  maybe because ptiViewSetOclProfiling api "
                 "not used prior to queue creations.\n\n";
  PTI_ASSERT(status == CL_SUCCESS);
  return start;
}
...
inline cl_int GetEventStatus(cl_event event) {
  ...
  cl_int status = clGetEventInfo(event, CL_EVENT_COMMAND_EXECUTION_STATUS, sizeof(cl_int),
                                 &event_status, nullptr);
  PTI_ASSERT(status == CL_SUCCESS);
  return event_status;
}

inline void GetTimestamps(cl_device_id device, cl_ulong* host_timestamp,
                          cl_ulong* device_timestamp) {
  PTI_ASSERT(device != nullptr);
  PTI_ASSERT(host_timestamp != nullptr);
  PTI_ASSERT(device_timestamp != nullptr);
  cl_int status = clGetDeviceAndHostTimer(device, device_timestamp, host_timestamp);
  PTI_ASSERT(status == CL_SUCCESS);
