 ...
 public:
  ...
  inline pti_result CallbackSubscribe(pti_callback_subscriber_handle* subscriber,
    ...
    if (collector_ && collector_->IsTracingOn() &&
      ...
      uint32_t subscriber_id = collector_->AddCallbackSubscriber(callback, user_data);
      if (subscriber_id == 0) {
    ...
    return PTI_ERROR_AT_LEAST_ONE_GPU_VIEW_MUST_BE_ENABLED;
  }

  inline pti_result CallbackUnsubscribe(pti_callback_subscriber_handle subscriber) {
    if (collector_) {
      auto result = collector_->RemoveCallbackSubscriber(subscriber);
      if (result != pti_result::PTI_SUCCESS) {
