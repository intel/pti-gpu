      ...
      case pti_view_kind::PTI_VIEW_DEVICE_GPU_MEM_COPY_P2P: {
        ...
        if (p2p_d2s_record) {
          if (rec->_memcpy_type == pti_view_memcpy_type::PTI_VIEW_MEMCPY_TYPE_D2S) {
            memcopy_type_valid = true;
            std::cout << "Stringified-memcoy: " << ptiViewMemcpyTypeToString(rec->_memcpy_type)
                      << " : expected-D2S\n";
...
void p2pTest() {
  ...
  try {
    ...
    single_context = context(gpu_devices);
    std::cout << "Number of Root Devices: " << num_root_devices << "\n";
    for (uint32_t i = 0; i < num_root_devices; i++) {
      gpu_queues.push_back(queue(single_context, gpu_devices[i]));
  ...
  if (num_root_devices > 1) {
    ...
    for (uint32_t i = 0; i < num_root_devices; i++) {
      auto hSrcDevice = get_native<sycl::backend::opencl>(gpu_queues[i].get_device());
      for (uint32_t j = 0; j < num_root_devices; j++) {
        if (i == j) {
          continue;
        }
        auto hDstDevice = get_native<sycl::backend::opencl>(gpu_queues[j].get_device());

        std::cout << "Src/Dst#: " << hSrcDevice << " : " << hDstDevice << std::endl;
        if (hSrcDevice && hDstDevice && (hSrcDevice != hDstDevice)) {
          // For OCL -- assume true if single context, distinct 2 devices.
    ...
    int* aD = malloc_device<int>(array_size, gpu_queues[0]);
    int* bD = malloc_device<int>(array_size, gpu_queues[1]);
        //"MemCopy D2D"
        std::cout << "===============================================> D2D - ints\n";
        gpu_queues[0].memcpy(aD, bD, array_size).wait();

        //"MemCopy D2D"
        std::cout << "===============================================> D2D\n:";
        std::cout << "src:dst -- " << connected_dev1 << " : " << connected_dev2 << "\n";
        ...
        //"MemCopy D2S"
        std::cout << "===============================================> D2S\n:";
        std::cout << "src:dst -- " << connected_dev2 << " : " << connected_dev1 << "\n";
        ...
        gpu_queues[connected_dev1]
            ...
            .wait();
        //"MemCopy S2S"
        std::cout << "src:dst -- " << connected_dev2 << " : " << connected_dev2 << "\n";
        gpu_queues[connected_dev2]
  ...
  for (uint32_t i = 0; i < num_root_devices; i++) {
    ...
    free(gpu_device_ptrs[i], single_context);
    free(gpu_shared_ptrs[i], single_context);
    free(host_ptrs[i], single_context);
  }
}
...
TEST_F(MemoryOperationFixtureTest, P2PMemoryCopyRecords) {
  // TODO-OCL -- turn this on when p2p is supported in opencl runtime.
  GTEST_SKIP();
