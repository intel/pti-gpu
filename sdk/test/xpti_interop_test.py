        ...
        for i in range(max_lines):
            ...
            if line1 != line2:
                analysis["differences"].append(
                    {"line": i + 1, label1: line1, label2: line2}
                )
...
def analyze_outputs_and_report_statistics(
    ret1, stdout1, stderr1, ret2, stdout2, stderr2
):
    """
    ...
    # Analyze stdout
    print(f"\nSTDOUT Analysis:")
    stdout_analysis = analyze_output_differences(
        stdout1, stdout2, "clean", "foreign_subscriber"
    )
    print(f"  Clean run output length: {stdout_analysis['line_count_1']} lines")
    print(
        f"  Foreign subscriber output length: {stdout_analysis['line_count_2']} lines"
    )
    print(f"  Outputs identical: {stdout_analysis['identical']}")
    print(
        f"  Sycl Runtime Records found (clean): {stdout_analysis['sycl_runtime_found_1']}"
    )
    print(
        f"  Sycl Runtime Records found (foreign): {stdout_analysis['sycl_runtime_found_2']}"
    )
    ...
    # Analyze stderr
    print(f"\nSTDERR Analysis:")
    stderr_analysis = analyze_output_differences(
        stderr1, stderr2, "clean", "foreign_subscriber"
    )
    print(f"  Clean run stderr length: {stderr_analysis['line_count_1']} lines")
    print(
        f"  Foreign subscriber stderr length: {stderr_analysis['line_count_2']} lines"
    )
...
def perform_behavioral_checks(ret1, ret2, stdout_analysis, stderr_analysis):
    ...
    for record_type in stdout_analysis["record_counts_1"].keys():
        if record_type not in [
            "Found Sycl Runtime Record"
        ]:  # Don't check Sycl Runtime Records as we expect them to differ
            count1 = stdout_analysis["record_counts_1"][record_type]
            count2 = stdout_analysis["record_counts_2"][record_type]
            if count1 != count2:
                print(
                    f"FAIL: {record_type} counts differ - Clean:{count1}, Foreign:{count2}"
                )
    ...
    REAL_REPETITIONS = (
        REPETITIONS + 1
    )  # Number of actual kernel launches expected, additional run is always added - it is warmup
    if kernel_count_clean == REAL_REPETITIONS:
        print(
            f"PASS: Clean run has correct number of kernel records ({kernel_count_clean} == {REAL_REPETITIONS})"
        )
    else:
        print(
            f"FAIL: Clean run kernel records mismatch - Expected:{REAL_REPETITIONS}, Got:{kernel_count_clean}"
        )
        test_passed = False

    if kernel_count_foreign == REAL_REPETITIONS:
        print(
            f"PASS: Foreign run has correct number of kernel records ({kernel_count_foreign} == {REAL_REPETITIONS})"
        )
    else:
        print(
            f"FAIL: Foreign run kernel records mismatch - Expected:{REAL_REPETITIONS}, Got:{kernel_count_foreign}"
        )
        test_passed = False
...
def save_detailed_outputs(stdout1, stdout2, stderr1, stderr2):
    ...
    with tempfile.NamedTemporaryFile(
        mode="w", suffix="_clean_stdout.txt", delete=False
    ) as f:
        f.write(stdout1)
        print(f"\nDetailed clean run stdout saved to: {f.name}")

    with tempfile.NamedTemporaryFile(
        mode="w", suffix="_foreign_stdout.txt", delete=False
    ) as f:
        f.write(stdout2)
    ...
    with tempfile.NamedTemporaryFile(
        mode="w", suffix="_clean_stderr.txt", delete=False
    ) as f:
        f.write(stderr1)
        print(f"Detailed clean run stderr saved to: {f.name}")

    with tempfile.NamedTemporaryFile(
        mode="w", suffix="_foreign_stderr.txt", delete=False
    ) as f:
        f.write(stderr2)
...
def validate_test_results(ret1, stdout1, stderr1, ret2, stdout2, stderr2):
    ...
    stdout_analysis, stderr_analysis = analyze_outputs_and_report_statistics(
        ret1, stdout1, stderr1, ret2, stdout2, stderr2
    )

    # Perform behavioral validation checks
    test_passed = perform_behavioral_checks(
        ret1, ret2, stdout_analysis, stderr_analysis
    )
...
def test_xpti_interoperability(executable_dir):
    ...
    ret1, stdout1, stderr1, ret2, stdout2, stderr2 = run_test_scenarios(
        str(executable_path)
    )
...
def main():
    if len(sys.argv) != 2:
        ...
        print(
            f"  The test will look for '{TARGET_EXECUTABLE}' in the provided directory."
        )
        return 1
