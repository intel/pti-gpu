...
static void Compute(const std::vector<float>& a,
  ...
  for (unsigned i = 0; i < repeat_count; ++i) {
    ...
    float eps = ComputeOnCPU(a, b, c, size, expected_result);
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Target device: CPU" << std::endl;
  ...
  Compute(a, b, c, size, repeat_count, expected_result);
  auto end = std::chrono::steady_clock::now();
  std::chrono::duration<float> time = end - start;

  std::cout << "Total execution time: " << time.count() << " ns" << std::endl;
  return 0;
}
